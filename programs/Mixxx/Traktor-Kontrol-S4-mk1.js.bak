...
TraktorKontrolS4mk1.init = function(id,debugging){
     ...
     for (var i = 0; i <= 3; i++) engine.setValue('[Channel'+(i+1)+']',"quantize",true);
     for (var i = 0; i <= 15; i++) engine.setValue('[Sampler'+(i+1)+']',"quantize",true);
};

TraktorKontrolS4mk1.fineRate = function(channel,control,value,status,group) {
       var actualRate = engine.getValue('[Channel' + (channel+1) + ']','rate');
       print('Actal rate: ' + actualRate);
       if (value > 1) engine.setValue('[Channel' + (channel+1) + ']','rate',actualRate + 0.001);
       else engine.setValue('[Channel' + (channel+1) + ']','rate',actualRate - 0.001);
}
...
TraktorKontrolS4mk1.rate = function(channel,control,value,status,group) {
       ...
       engine.setValue('[Channel' + (channel+1) + ']','rate',rateSkip);
...
TraktorKontrolS4mk1.quantize = function(channel,control,value,status,group) {
      if (value) {
          ...
          midi.sendShortMsg(0xB4,0x05,quantizeVal);
...
// Reset gain
TraktorKontrolS4mk1.resetGain = function(channel,control,value,status,group) {
      if (value) {
          print("Ajustando pregain")
...
TraktorKontrolS4mk1.beatLoopSize = function(channel,control,value,status,group) {
      var actualBeatLoop = engine.getValue('[Channel'+(channel+1)+']','beatloop_size');
      ...
      print('beatId: '+beatId)
      if (value < 63) {
          print('Beat loop size backWard: '+engine.getValue('[Channel'+(channel+1)+']','beatloop_size'));
          if (beatId) engine.setValue('[Channel'+(channel+1)+']','beatloop_size',beatJumps[beatId - 1]);
          midi.sendShortMsg(0xB0+channel,0x15,63);
      }
      if (value > 63) {
          if (beatId < 11) engine.setValue('[Channel'+(channel+1)+']','beatloop_size',beatJumps[beatId+ 1]);
          midi.sendShortMsg(0xB0+channel,0x15,63);
      }
}
...
TraktorKontrolS4mk1.beatJumpSize = function(channel,control,value,status,group) {
      var actualBeatJump = engine.getValue('[Channel'+(channel+1)+']','beatjump_size');
      var beatId = 0
      for (var i = 0; i <= 11; i++){
           if (actualBeatJump == beatJumps[i])  beatId = i;
      }
      if (value < 63) {
          if (beatId) engine.setValue('[Channel'+(channel+1)+']','beatjump_size',beatJumps[beatId - 1]);
          midi.sendShortMsg(0xB0+channel,0x13,63);
      }
      if (value > 63) {
          if (beatId < 11) engine.setValue('[Channel'+(channel+1)+']','beatjump_size',beatJumps[beatId+ 1]);
          midi.sendShortMsg(0xB0+channel,0x13,63);
      }
}
...
TraktorKontrolS4mk1.playStopPreview = function(channel,control,value,status,group) {
    ...
    if (value) browseKnobPressed = true; else browseKnobPressed = false;
...
TraktorKontrolS4mk1.maximize_library = function(channel,control,value,status,group) {
    if (value) browseButtonFlag = true; else browseButtonFlag = false;
...
TraktorKontrolS4mk1.PFL = function(channel,control,value,status,group) {
         ...
         var pfl = engine.getParameter('[Channel'+(channel+1)+']',"pfl");
         ...
         if ((engine.getParameter('[Master]','maximize_library') || browseButtonFlag)  && value) {
             engine.setValue('[Channel'+(channel+1)+']',"LoadSelectedTrack",true);
          ...
          // del canal correspondiente
              ...
              if (pfl && !value) {
                    engine.setValue('[Channel'+(channel+1)+']',"pfl",false)
                    PFLs[channel] = false
                 }
              if (!pfl && !value) {
                    engine.setValue('[Channel'+(channel+1)+']',"pfl",true)
                    PFLs[channel] = true
                 }
           }
}
...
TraktorKontrolS4mk1.eject = function(channel,control,value,status,group) {
          engine.setValue('[Channel'+(channel+1)+']',"eject",true);
          engine.setValue('[Channel'+(channel+1)+']',"LoadSelectedTrack",0x00);
